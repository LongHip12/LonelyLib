...
local v={}
v.__index=v
local w=function(w,x)
for y in x:gmatch"[^%.]+"do
w=w[y]
end
return w
end
local x=function(x,y,z,A)
if not A then
A=s.CurrentTheme
end
x[y]=w(A,if type(z)=="function"then z()else z)
end
local y=function(y,z,A)
for B,C in A do
x(z,B,C,y)
end
end
...
function v:destroy()
local C=self.Parent and self.Parent.Descendants and table.find(self.Parent.Descendants,self)
if C then
...
self.Position=self.Position:Lerp(V,Q)
end
local U=function()
while I==self do
if(tick()-R)>=1 then
...
function ar:Button(au)
return ar.NewOption(self,au)
end
...
if self.SET_ENABLED_OPTIONS then
...
self:NewOptions(table.unpack(au))
elseif type(au)=="function"then
table.insert(self.CALLBACKS,au)
end
end
...
an.Set=an.SetValue
ao.Set=ao.SetValue
ai.Set=function(self,v1,v2)
if v1 and v2 then
self:SetTitle(v1)
self:SetDescription(v2)
elseif v1 then
self:SetDescription(v1)
end
return self
end
ai.SetDesc=ai.SetDescription
ai.SetText=ai.SetDescription
ai.SetContent=ai.SetDescription
end
local al=function(al,am,an)
...
local at=ao:FindFirstChild("Title")
if at then
ar.Visible=as
...
local ar=E("TextButton",DropdownsHolder,{
Size=UDim2.fromScale(1,1),
BackgroundTransparency=1,
Active=true,
Text=""
...
local a_=aZ:FindFirstChildWhichIsA("TextLabel")
local a0=aZ:FindFirstChildWhichIsA("Frame")
if a_ and a0 then
...
local at=K({"number","string","nil","boolean","table"},true)
...
local aTab=self
local aSectionMeta={}
aSectionMeta.__index=function(t,k)
local v=ai[k]
...
ah.AddLeftGroupbox=ah.AddSection
ah.AddRightGroupbox=ah.AddSection
...
local aL
local aM_OnlineLabel
local aM_MembersLabel
if aA or aB then
aL=E("Frame",aG,{
...
if aA~=nil then
local OnlineFrame=aM(Color3.fromRGB(67,181,129),N(aA).." Online")
aM_OnlineLabel=OnlineFrame and OnlineFrame:FindFirstChildWhichIsA("TextLabel")
end
if aB~=nil then
local MembersFrame=aM(Color3.fromRGB(86,101,105),N(aB).." Members")
aM_MembersLabel=MembersFrame and MembersFrame:FindFirstChildWhichIsA("TextLabel")
end
end
...
Kind="DiscordInvite",
Parent=self,
SetMembers=function(self,num)
if aM_MembersLabel then
aM_MembersLabel.Text=N(num).." Members"
end
end,
SetOnline=function(self,num)
if aM_OnlineLabel then
aM_OnlineLabel.Text=N(num).." Online"
end
end
},ai)
end
...
aL.Opened=aO
aL.SET_ENABLED_OPTIONS=function(enabledList)
for _,optName in enabledList do
local opt=aS[tostring(optName)]
if opt and opt.Instance then
opt.Instance.Visible=true
end
end
end
local aT=function(aT,aU)
...
local aw=T[self]
assert(aw,`"Tab.SetTitle". tab not found`)
local ax=aw.SelectTabButton.Title
ax.Text=av
self.Title=av
end
...
function ag:SaveFlags()
if self._flush_flags then self._flush_flags() end
end
function ag:SaveSettings()
if self._flush_settings then self._flush_settings() end
end
function ag:CheckFlag(av)
return type(av)=="string"and ae[av]~=nil
end
function ag:GetSetting(av,aw)
local val=ae[av]
...
function ag:AddMinimizeButton(av)
local btn=E("ImageButton",I,{
Size=UDim2.fromOffset(35,35),
...
D.Draggable(btn,ad,0.5)
local Stroke,Corner
if av.Corner then
Corner=E("UICorner",btn,av.Corner)
end
if av.Stroke then
Stroke=E("UIStroke",btn,av.Stroke)
end
if av.Button and type(av.Button)=="table"then
for k,v in pairs(av.Button)do
btn[k]=v
end
end
u(btn.Activated,function()
ag:Minimize()
end)
return{
Stroke=Stroke,
...
ag.SetDefaultNotifyIcon=ag.SetNotifyDefaultIcon
ag.GetTabByName=ag.GetTabByTitle
ag.Notificafion=ag.Notify
ag.MinimizeBtn=ag.MinimizeButton
ag.CloseBtn=function(self)
self:Dialog({
Title="Close Window?",
Content="You Want Close UI?",
Options={
{Title="Yes",Callback=function()self:Destroy()end},
{Title="No"}
}
})
end
ag.Set=ag.SetSubTitle
R.Window=ag
end
...
ScriptFolder=ab[3]or ab.ScriptFolder or ab.FolderName or ab.SaveFolder
}
...
self.Flags=ag
self._flush_flags=ag_flush_fn
self._flush_settings=af_flush_fn
_G.SaveData=ag
getfenv().SaveSettings=function()
if ag_flush_fn then ag_flush_fn() end
end
getfenv().LoadSettings=function()
return true
end
getfenv().GetSetting=function(name,default)
local val=ag[name]
if val~=nil then return val end
return default
end
