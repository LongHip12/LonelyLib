...
local T1UIColor = {
	...
	["Button Background Color"] =  ColorSequence.new{
...
local function GetConfigPath()
    local folder = getgenv().ConfigFolder or "Lonely Hub"
    local file = getgenv().ConfigFile or "config"
    local username = game.Players.LocalPlayer.Name
    return folder .. "/" .. username .. "-" .. file .. ".json"
end
    ...
    if #keypoints < 2 then
        if #keypoints == 0 then
            ...
            return ColorSequence.new({
                ...
                ColorSequenceKeypoint.new(1, singleColor)
            })
        end
    end
...
function Library:CreateWindow(Setting)
    ...
    getgenv().ConfigPath = ConfigPath
    getgenv().ConfigFile = Setting.File or "config"
    
    LoadSettings()
    
    if getgenv().Themes and getgenv().Themes[ThemeName] then
        getgenv().UIColor = getgenv().Themes[ThemeName]
    end
	...
	function Main_Function:AddTab(PageName)
		...
		function pageFunction:AddSectiontwo(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			function sectionFunction:AddToggle(Setting)
				...
				check.BackgroundColor3 = getgenv().UIColor["Toggle Checked Color"] or Color3.fromRGB(255, 80, 80)
                ...
                function toggleFunction:SetTitle(newTitle)
                    ToggleTitle.Text = tostring(newTitle)
                end
                function toggleFunction:SetDescription(text)
                    if ToggleDesc then ToggleDesc.Text = tostring(text) end
                end
                toggleFunction.SetDesc = toggleFunction.SetDescription
                toggleFunction.SetContent = toggleFunction.SetDescription
                ...
                function toggleFunction:SetVisible(state)
                    ToggleFrame.Visible = state
                end
                function toggleFunction:Destroy()
                    ToggleFrame:Destroy()
                end
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            local gradientData = getgenv().UIColor["Button Background Color"]
            
            if type(gradientData) == "table" and gradientData.Color then
                UIGradient_1.Color = gradientData.Color
                UIGradient_1.Rotation = gradientData.Rotation or 0
            else
                UIGradient_1.Color = gradientData or ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 100, 100)),
                    ColorSequenceKeypoint.new(1, Color3.fromRGB(180, 30, 30))
                })
			...
			function sectionFunction:AddLabel(Setting)
                ...
                LabelList.Parent = LabelBG
                LabelList.SortOrder = Enum.SortOrder.LayoutOrder
                LabelList.Padding = UDim.new(0, 0)
                ...
                TitleLabel.Size = UDim2.new(1, -24, 0, 0)
                TitleLabel.Font = Enum.Font.GothamMedium
                TitleLabel.Text = Title
                TitleLabel.TextColor3 = Color3.fromRGB(240, 240, 230)
                ...
                local pad_top = Instance.new("UIPadding", TitleLabel)
                pad_top.PaddingTop = UDim.new(0, 6)

                if Desc and Desc ~= "" then
                    DescLabel.Name = "DescLabel"
                    DescLabel.Parent = LabelBG
                    ...
                    DescLabel.Text = Desc
                    DescLabel.TextColor3 = getgenv().UIColor["Toggle Desc Color"]
                    DescLabel.TextSize = 12
                    DescLabel.TextStrokeTransparency = 1
                    DescLabel.TextWrapped = true
                    DescLabel.TextXAlignment = Enum.TextXAlignment.Left
                    DescLabel.BackgroundColor3 = Color3.fromRGB(163,162,165)
                    DescLabel.RichText = true

                    local pad_top2 = Instance.new("UIPadding", TitleLabel)
                    local pad_bot = Instance.new("UIPadding", DescLabel)
                    pad_bot.PaddingBottom = UDim.new(0, 6)
                else
                    local pad_bot = Instance.new("UIPadding", TitleLabel)
                    pad_bot.PaddingBottom = UDim.new(0, 6)
                end
				...
				function labelFunction:SetDescription(text)
					if Desc ~= nil then
						DescLabel.Text = tostring(text)
					end
				end
				labelFunction.SetDesc = labelFunction.SetDescription
				labelFunction.SetContent = labelFunction.SetDescription
				function labelFunction.SetColor(color)
					TitleLabel.TextColor3 = color
				end
				function labelFunction:SetVisible(state)
...
function sectionFunction:AddDropdown(Setting)
    ...
    if Selected then
        ...
        for _, v in ipairs(Setting.Values) do
            List[v] = savedData[v] or false
        end
    end
    
    local Default = GetSetting(Flag, (function ()
        ...
        elseif Slider then
            ...
            for i, v in pairs(ListNew) do
                ...
                RunService.RenderStepped:Connect(function()
                    ...
                    if dragging and dragInput then
                        local percentage = (dragInput.Position.X - Bar.AbsolutePosition.X) / SliderBar.AbsoluteSize.X
                        percentage = math.clamp(percentage, 0, 1)
                        local value = minValue + (maxValue - minValue) * percentage
                        
                        if v.Rouding then
                            value = tonumber(string.format("%.".. v.Rouding .."f", value))
                ...
                local sliderControlData = {
                    ...
                    TabName = Page_Name,
                    TabButton = PageName,
                    GetValue = function() return tonumber(Sliderbox_2.Text) or minValue end,
                    Type = "Slider"
                }
                table.insert(getgenv().AllControls, sliderControlData)
            end
    ...
    function dropdownFunction:SetValue(value)
        if Selected then
            ...
            refreshlist()
            if Callback then
                pcall(Callback, self:GetValue())
            end
        elseif Slider then
            if type(value) == "table" then
                for i, v in pairs(ListNew) do
                    if value[v.Title] ~= nil then
                        v.Default = value[v.Title]
                    end
                end
            end
            refreshlist()
    ...
    local controlData = {
        ...
        TabName = Page_Name,
        TabButton = PageName,
        GetValue = function() return dropdownFunction:GetValue() end,
        SetValue = function(_, value) dropdownFunction:SetValue(value) end,
        Type = Selected and "MultiDropdown" or (Slider and "SliderDropdown" or "Dropdown")
    }
    table.insert(getgenv().AllControls, controlData)
    
    function dropdownFunction:SetTitle(text)
        if Search then
            Dropdowntitle.PlaceholderText = tostring(text)
        else
            Dropdowntitle.Text = tostring(text)
        end
        Title = tostring(text)
    end
    function dropdownFunction:SetVisible(state)
        DropdownFrame.Visible = state
    end
    function dropdownFunction:Destroy()
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    if type(Default) == "table" then
        CurrentKey = GetKeyString(Default.key or "F")
        CurrentMode = Default.mode or Mode
    else
        CurrentKey = GetKeyString(Default or "F")
        CurrentMode = Mode
    end
    ...
    local function GetKeyBindValue()
        return {
            key = CurrentKey,
            mode = CurrentMode,
            state = CurrentMode == "Toggle" and ToggleState or HoldActive
        }
    end
    ...
    local controlData = {
        ...
        TabName = Page_Name,
        TabButton = PageName,
        GetValue = GetKeyBindValue,
        Type = "Keybind"
    }
    table.insert(getgenv().AllControls, controlData)
    
    local keybindFunction = {}
    function keybindFunction:SetTitle(newTitle)
        BindButtonTitle.Text = tostring(newTitle)
    end
    ...
    function keybindFunction:SetVisible(state)
        BindFrame.Visible = state
    end
    function keybindFunction:Destroy()
        BindFrame:Destroy()
    end
    ...
    function textbox_function:SetVisible(state)
        BoxFrame.Visible = state
    end
    function textbox_function:Destroy()
        BoxFrame:Destroy()
    end
    local controlData = {
        Name = TitleText,
...
function sectionFunction:AddInput(idk, Setting)
    ...
    return textbox_function
        ...
        function sectionFunction:AddSlider(Setting)
            ...
            RunService.RenderStepped:Connect(function()
                ...
                if dragging and dragInput then
                    ...
                    if Setting.Rouding then
                        value = tonumber(string.format("%.".. Setting.Rouding .."f", value))
                    elseif not Precise then
                        value = math.floor(value)
                    end
            ...
            function slider_function:SetVisible(state)
                SliderFrame.Visible = state
            end
            function slider_function:Destroy()
                SliderFrame:Destroy()
            end
        ...
        function pagefunc:AddLeftGroupbox(name)
            return pageFunction:AddSectiontwo(name)
        end
        ...
        function pagefunc:SetTitle(text)
            TabTitle.Text = tostring(text)
        end
        function pagefunc:SetVisible(state)
            PageName.Visible = state
            PageContainer.Visible = state
        end
		return pagefunc
        end
